...
module.exports.saveAccessToken = function(accessToken, clientID, expires, user, callback){
	...
	console.log(qs);
    var docRef  = db.collection('Authentication').doc('CultureCollectionAdmin').collection('Tokens').doc(user.id);
...
module.exports.getAccessToken = function(token, callback) {
    ...
    var docRef  = db.collection('Authentication').doc('CultureCollectionAdmin').collection('Tokens').where('accessToken','==', token);
