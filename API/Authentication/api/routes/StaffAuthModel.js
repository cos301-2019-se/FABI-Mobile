...
module.exports.saveAccessToken = function(accessToken, clientID, expires, user, callback){
	...
	console.log(qs);
    var docRef  = db.collection('Authentication').doc('Staff').collection('Tokens').doc(user.id);
...
module.exports.getAccessToken = function(token, callback) {
    ...
    var docRef  = db.collection('Authentication').doc('Staff').collection('Tokens').where('accessToken','==', token);
